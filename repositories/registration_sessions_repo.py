...
from infra.db import get_cursor, transaction
    ...
    with get_cursor() as cur:
        cur.execute(
            ...
            INSERT INTO public.attendance_registration_sessions (
                bot_owner, tg_id, private_chat_id, stage,
                ...
                %s, %s, %s, 'awaiting_input',
                NULL, NULL, NULL,
                %s, %s, %s, %s, NULL
            )
            ON CONFLICT (bot_owner, tg_id) DO UPDATE SET
    ...
    *,
    ...
    with get_cursor() as cur:
        cur.execute(
            ...
            """,
            (bot_owner, int(tg_id), int(private_chat_id), now, now, now),
        )
        cur.execute(
            """
            SELECT 1
...
def clear_session(*, bot_owner: str, tg_id: int) -> None:
    with get_cursor() as cur:
        cur.execute(
            """
            DELETE FROM public.attendance_registration_sessions
            WHERE bot_owner = %s AND tg_id = %s
            """,
            (bot_owner, int(tg_id)),
        )
    ...
    *,
    ...
    with transaction() as cur:
        ...
        if (
            ...
            _delete_session_cur(cur, bot_owner=bot_owner, tg_id=tg_id)
            return ConfirmResult(code="expired")

        if registrations_repo.get_by_tg_id_cur(cur, tg_id=int(tg_id)) is not None:
            _delete_session_cur(cur, bot_owner=bot_owner, tg_id=tg_id)
...
def _delete_session_cur(cur, *, bot_owner: str, tg_id: int) -> None:
    cur.execute(
        """
        DELETE FROM public.attendance_registration_sessions
        WHERE bot_owner = %s AND tg_id = %s
