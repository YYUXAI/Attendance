    ...
    if _remote_diff_desktop_fallback_enabled():
        ...
        try:
            ...
            try:
                ...
                pass
            log.exception("checkin_zhipu: remote_diff http %s model=%s body=%s", status, model, text_summary(err_body))
        ...
        if data is None:
            log.warning("checkin_zhipu: remote_diff invalid json raw=%s", text_summary(raw))
...
async def extract_checkin_from_zhipu_vision(
    *,
    ...
    log.info(
        ...
        "visible_texts=%s chat_ref=%s sender_candidates=%s",
        model,
        len(prepared) // 1024,
        use_visible_texts,
    ...
    try:
        ...
        try:
            ...
            pass
        log.exception("checkin_zhipu: http %s model=%s body=%s", status, model, text_summary(err_body))
        if status in {401, 403}:
            return None, CheckinAiExtractError(
                "AI_AUTH_FAILED",
    ...
    if extraction is None:
        log.warning("checkin_zhipu: invalid json raw=%s", text_summary(raw))
